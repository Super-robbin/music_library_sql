...
class ArtistRepository:
    ...
    def find(self, artist_id):
        ...
        # %s is a placeholder, similar idea of fstring, and then
        # we put the value we want to use in [artist_id], which is
        # a list of all the values that should go into the placeholder
        row = rows[0]
        return Artist(row["id"], row["name"], row["genre"])

    # Find a single artist, along with its albums
